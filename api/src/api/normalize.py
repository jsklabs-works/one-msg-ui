...
def rabbitmq_resource(r, broker_id: str) -> models.Resource:
    return models.Resource(
        id=r.id,
        broker_id=broker_id,
...
def rabbitmq_health_event(h, broker_id: str) -> models.HealthEvent:
    return models.HealthEvent(
        broker_id=broker_id,
        system_type="rabbitmq",
        severity=h.severity.value if hasattr(h.severity, "value") else h.severity,
...
def rabbitmq_message_sample(m) -> models.MessageSample:
    return models.MessageSample(
        resource_id=m.resource_id,
        message_id=m.message_id,
        timestamp=m.timestamp,
