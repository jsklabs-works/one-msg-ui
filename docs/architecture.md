...
**Scope:** A single console covering IBM MQ, Apache Kafka, Solace PubSub+, and (added in Phase 5, see §6) RabbitMQ, showing queue/topic depth, consumer lag, broker/system health, **and read-only message browsing/inspection** (peek at what's sitting on a queue/topic without consuming it) — the goal is to ease day-to-day messaging operations and support, not just provide a monitoring view.
