...
**Scope:** A single console covering IBM MQ, Apache Kafka, Solace PubSub+, and (added in Phase 5, see §6) RabbitMQ and ActiveMQ Artemis, showing queue/topic depth, consumer lag, broker/system health, **and read-only message browsing/inspection** (peek at what's sitting on a queue/topic without consuming it) — the goal is to ease day-to-day messaging operations and support, not just provide a monitoring view.
...
- **Solace's VPN-level spool fields mix units in the same object.** `msgSpoolUsage` (current usage) is in bytes; `maxMsgSpoolUsage` (the configured quota) right next to it is in MB. Comparing them directly reported a VPN with 506 bytes spooled against a 100 MB quota as 506% full/critical, when the broker's own console showed 0.0005 MB used — caught by cross-checking against the real admin console, not something the API schema itself hints at. See [`adapters/solace/README.md`](../adapters/solace/README.md).
...
4. **Phase 4 — Admin actions, produce/consume test tooling.** Not started — deprioritized in favor of adding more broker types first (see Phase 5), since that's where the actual demand showed up.
5. **Phase 5 — Add more broker types, pulled forward ahead of the rest of Phase 4.** RabbitMQ done: the management HTTP API alone covers monitoring *and* non-destructive peek (`ackmode: ack_requeue_true`) — no AMQP client needed at all. See [`adapters/rabbitmq/README.md`](../adapters/rabbitmq/README.md). ActiveMQ Artemis done too: Jolokia (JMX-over-HTTP, bundled with the broker's own web console) covers monitoring and peek (the queue MBean's `browse()` operation) the same way — no JMS/core client needed. See [`adapters/activemq/README.md`](../adapters/activemq/README.md). Azure Service Bus and SQS/SNS remain further out (no local Docker broker — verification would need a cloud namespace or an emulator, a different verification story than the Docker-broker pattern every adapter so far has used).

All five broker adapters (Phases 1-3, 5) are feature-complete and wired end-to-end through the unified API layer and UI.
